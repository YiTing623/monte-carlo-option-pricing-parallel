...
// -----------------------------------------------------------------
// Function: monteCarloPriceParallel
// Purpose:  Uses OpenMP to parallelize the Monte Carlo simulation to
//           calculate the price of a European call option by averaging
//           discounted payoffs.
// -----------------------------------------------------------------
double monteCarloPriceParallel(int numSimulations, double S0, double K, double r, double sigma, double T, int steps) {
    double totalPayoff = 0.0;

    #pragma omp parallel
    {
        // Each thread gets its own random number generator seeded with current time and its thread ID
        std::default_random_engine generator(static_cast<unsigned>(time(nullptr)) + omp_get_thread_num());

        #pragma omp for reduction(+:totalPayoff)
...
int main() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        double mcPrice = monteCarloPriceParallel(numSimulations, S0, K, r, sigma, T, steps);
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed = end - start;

        std::cout << "Runtime: " << elapsed.count() << " seconds\n";
        std::cout << "Monte Carlo Price (Parallel): " << mcPrice << "\n";
